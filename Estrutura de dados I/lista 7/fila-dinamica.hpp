...
void mostraF(Fila *f)
{
    ...
    if(vaziaF(f) == false) //verifica se a fila não está vazia
    {
        ...
        {
            cout << "CPF/CNPJ: " << no->dado.cpfCnpj << endl;
            if(no->dado.tipoPessoa == 'f'){
            cout << "Pacote de revisão: " << no->dado.pacoteRevisaoP << endl;
            }
            cout << "Placa do veículo " << no->dado.placaVeiculo << endl;
            cout << "Tipo de Pessoa: " << no->dado.tipoPessoa << endl;
            no = no->prox;
...
bool buscaF(Fila *f, Ficha dado)
{
    ...
    while (no != NULL) //faça enquanto (no != NULL)
    {
        if(no->dado.cpfCnpj == dado.cpfCnpj)
            return true;
