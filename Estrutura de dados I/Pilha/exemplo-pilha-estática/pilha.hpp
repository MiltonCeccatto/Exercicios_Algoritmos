...
bool verificaInicializacaoPilha(Pilha *p) //verifica de a pilha foi inicializada
{
    if(p->dados != NULL)
        return true;
    else
...
///push
bool empilhar(Pilha *p, int valor) ///push
{
    /// retorna false se a pilha não foi inicializada ou se ela está cheia
    if (verificaInicializacaoPilha(p)==false || cheiaP(p)==true)
        return false;
    else{
        p->topo++;
...
bool buscarPilha(Pilha *p, int valor)
{
